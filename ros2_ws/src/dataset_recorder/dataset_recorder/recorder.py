...
class SampledSyncRecorder(Node):
    def __init__(self):
        ...
        ats = ApproximateTimeSynchronizer(
            [sub_color, sub_left, sub_cctv, sub_arm], queue_size=10, slop=0.05, allow_headerless=False
        )
        ...
        # sampling timer stored for cancelation
    ...
    def handle_start(self, req, res):
        # create a new executor for this episode
    ...
    def handle_stop(self, req, res):
        ...
        # stop the timer and recording flag
    ...
    def _cb_sample(self):
        # guard: only record when flag is set
        if not self.is_recording or not self.h5:
            return

        # ensure all images present
        if any(self.img_buf[k] is None for k in self.img_buf):
            return
        ...
        if self.joint_initialized and j:
            ...
            lin_sp = np.linalg.norm(pos)
            ang_sp = np.linalg.norm(twist)
