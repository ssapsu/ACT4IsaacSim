...
if __name__ == "__main__":
    ...
    # 3D 입력: (batch, seq_len, d_model)
    src = torch.rand(batch_size, seq_len, d_model)
    mask = torch.zeros(batch_size, seq_len, dtype=torch.bool)

    # query_embed: (num_queries, d_model)
    query_embed = torch.rand(num_queries, d_model)
    # pos_embed: (seq_len, d_model) — 여기가 핵심!
    pos_embed = torch.rand(seq_len, d_model)
    ...
    # 추가 입력 없이 호출
    output = transformer(src, mask, query_embed, pos_embed)
    print(output.shape)
